...
export default function TerminalDashboard() {
  ...
  const handleMessage = useCallback((msg: Record<string, unknown>) => {
    ...
    if (msg.type === "tick") {
      applyTickUpdate(msg, {
        setBook, setTick, setMarket, setPriceHistory, setChartBars, setPricePoint,
      });
      syncMeta(msg);
  ...
  const handleBbFunction = useCallback((fn: BbFunction, sym?: string) => {
    ...
    if (fn === "HP") {
      changeTimeframe("1Y");
      window.open(chartPageUrl(sym || symbol, "1Y"), "_blank", "noopener,noreferrer");
    }
    ...
    <div className="bb-terminal">
      ...
      <nav className="mobile-desk-nav" aria-label="Desk panels">
        {(["chart", "market", "report", "ibank", "research"] as const).map((tab) => (
          ...
          >
            {tab === "ibank" ? "I-Bank" : tab.charAt(0).toUpperCase() + tab.slice(1)}
          </button>
        ))}
      </nav>
...
const MemoChart = memo(BloombergTerminalChart, (a, b) =>
  a.symbol === b.symbol && a.timeframe === b.timeframe && a.intervalLabel === b.intervalLabel
  && a.loading === b.loading && a.symbolLoading === b.symbolLoading && a.bars === b.bars
  && a.onTimeframeChange === b.onTimeframeChange && a.onSymbolChange === b.onSymbolChange
  && a.tick?.price === b.tick?.price && a.tick?.change === b.tick?.change
  && a.tick?.change_pct === b.tick?.change_pct && a.market?.status === b.market?.status,
);
